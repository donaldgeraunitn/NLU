...
class Lang:
    def __init__(self, intents, slots):
        self.slot2id = {slot: idx for idx, slot in enumerate(sorted(slots))}
        self.intent2id = {intent: idx for idx, intent in enumerate(sorted(intents))}
...
class IntentsAndSlots(data.Dataset):
    def __init__(self, dataset, lang):
        ...
        for example in dataset:
            ...
            self.intents.append(lang.intent2id[example["intent"]])
...
def load_dataset(dataset_dir="dataset/ATIS"):
    train_raw = load_data(os.path.join(dataset_dir, "train.json"))
    test_raw = load_data(os.path.join(dataset_dir, "test.json"))
    return train_raw, test_raw


def split_train_dev(tmp_train_raw, dev_size=0.10, seed=42):
    intents = [example["intent"] for example in tmp_train_raw]
    ...
    for example in tmp_train_raw:
        intent = example["intent"]
        if intent_counts[intent] > 1:
            inputs.append(example)
            labels.append(intent)
        else:
            train_only.append(example)
    ...
    train_raw.extend(train_only)

    return train_raw, dev_raw


def make_datasets(dataset_dir="dataset/ATIS", dev_size=0.10, data_seed=42):
    tmp_train_raw, test_raw = load_dataset(dataset_dir)
    train_raw, dev_raw = split_train_dev(tmp_train_raw, dev_size, data_seed)

    corpus = train_raw + dev_raw + test_raw
    slots = set(sum([example["slots"].split() for example in corpus], []))
    intents = set(example["intent"] for example in corpus)
...
def _align_slot_labels(slot_labels, word_ids, slot2id, model_type):
...
def _collate_bert(batch, tokenizer, lang, device, max_length):
    ...
    for idx, sample in enumerate(batch):
        ...
        aligned_labels = _align_slot_labels(
            sample["slots"], word_ids, lang.slot2id, model_type="bert"
        )
...
def _collate_gpt2(batch, tokenizer, lang, device, max_length):
    if max_length < 2:
        raise ValueError("max_length must be at least 2 for GPT2.")
    ...
    for sample in batch:
        ...
        aligned_labels = _align_slot_labels(
            sample["slots"], word_ids, lang.slot2id, model_type="gpt2"
        )
        truncated_words = _truncate_words(sample["utterance"], word_ids)
...
def make_dataloaders(
    ...
    train_dataset, dev_dataset, test_dataset, lang = make_datasets(
