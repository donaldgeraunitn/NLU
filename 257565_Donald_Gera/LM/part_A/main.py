...
# Candidate values used by the predefined experiments unless overridden from the CLI.
...
# Checkpoints store the class name so evaluation can rebuild the correct model variant.
MODEL_REGISTRY = {
...
def parse_args():
    # The same entry point supports both staged training experiments and evaluation-only execution.
    ...
    # Output paths and tokenizer checkpoint.
    ...
    # Search spaces can be overridden without modifying the source code.
    ...
    # Shared training hyperparameters used by every candidate model.
    ...
    # Later stages read the reports generated by earlier stages of the search.
    ...
    # Evaluation mode skips training and loads a previously saved checkpoint.
...
def get_device(args):
    # Prefer the first CUDA device when available, while remaining runnable on CPU.
...
def get_output_dir(args):
    # Keep the outputs of different experiment stages separated by default.
...
# Report helpers allow later stages to reuse the best settings saved by previous stages.
...
def get_learning_rate_or_raise(args, *reports):
    # A manually provided learning rate has priority over values stored in reports.
...
# Forward the common training arguments to each experiment without duplicating them.
...
def run_greedy_experiment(args, tokenizer, device, output_dir):
    # Start from the baseline LR-search result and optimize one architecture value at a time.
    ...
    # The selected value from each step becomes fixed during the next step.
    search_space = [
    ...
    for parameter_name, candidate_values in search_space:
        # Only the current parameter varies; all previously selected values remain fixed.
        print("\n" + "=" * 70)
        ...
        # Propagate the winning candidate to the following greedy step.
    ...
    # Evaluate and store the architecture selected after the final greedy step.
...
def evaluate_checkpoint(args, tokenizer, device):
    # Evaluation-only mode rebuilds the saved architecture and runs it on the test split.
    if args.checkpoint is None:
        raise ValueError("Use --checkpoint PATH together with --eval.")

    checkpoint = torch.load(args.checkpoint, map_location=device)

    # The fallbacks also allow loading plain state dictionaries or older checkpoints.
...
def main():
    # Main dispatch: evaluate a saved model or execute one selected experiment stage.
    args = parse_args()
    set_seed(args.seed)
    ...
    if args.eval:
        # No training datasets or output directories are needed in evaluation-only mode.
    ...
    # Load once here to print dataset statistics; dataloaders are built inside training helpers.
    ...
    # Arguments shared by all single-parameter experiment stages.
    common = {
        "tokenizer": tokenizer,
        "make_dataloaders_fn": make_dataloaders,
        "device": device,
    ...
    if args.experiment == "baseline_lr":
        # Step 1: select the learning rate for the initial small architecture.
        ...
        # Step 2: tune the architecture dimensions sequentially.
        report = run_greedy_experiment(args, tokenizer, device, output_dir)

    elif args.experiment == "final_lr":
        # Step 3: repeat LR tuning after the architecture search.
        baseline_report = load_report(args.baseline_report)
        ...
        # Step 4: evaluate dropout values using the selected architecture and LR.
        baseline_report = load_report(args.baseline_report)
        hparams_report = load_report(args.hparams_report)
        ...
        # Step 5: add weight tying to the best available dropout or non-dropout variant.
        baseline_report = load_report(args.baseline_report)
        hparams_report = load_report(args.hparams_report)
        final_lr_report = load_report(args.final_lr_report)
        ...
        # Preserve dropout only when it improved or matched validation perplexity.
